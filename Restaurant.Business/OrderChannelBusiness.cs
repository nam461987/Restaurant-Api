...
namespace Restaurant.Business
{
    public class OrderChannelBusiness : IOrderChannelBusiness
    {
        ...
        public async Task<IPaginatedList<OrderChannelDto>> GetAll(int restaurantId, int branchId, int pageIndex, int pageSize)
        {
            var OrderChannelRepo = _orderChannelRepository.Repo;

            var result = await (from orderChannel in OrderChannelRepo
        ...
        public async Task<List<OrderChannel>> GetAllNotPaginate(int restaurantId, int branchId)
        {
            var result = await _orderChannelRepository.Repo.ToFilterByRole(f => f.RestaurantId, null, restaurantId, 0)
